...
def do_analysis(guards):
    ...
    for cutoff in xrange(0, THRESHOLD_MAX, THRESHOLD_STEP):
        ...
        guards_n.append(len(guards))
        total_bw.append(guards.get_total_bw())

    return guards_n, cutoffs
...
def main():
    ...
    ax.set_title("Number of guards remaining over different bw cutoffs")
    plt.xlabel("Bandwidth cutoff in kB/s")
    plt.ylabel("Number of guard nodes")
